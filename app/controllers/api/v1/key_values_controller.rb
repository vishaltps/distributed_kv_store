module Api
	module V1
		class KeyValuesController < ApplicationController
			...
			def destroy
				begin
					result = ActiveRecord::Base.connection.execute("UPDATE stores SET ttl = -1 WHERE key = '#{params[:key]}' AND ttl > extract(epoch from ((now() at time zone 'UTC')))")
			...
			def show
				begin
					...
					if result.cmd_tuples.positive?
						...
						render json: { error_messages: 'Record not found' }, status: 404
					end
				rescue StandardError => e
					render json: { error_messages: "Record couldn't fetch, error: #{e.message}"}, status: 500
				end
			end
